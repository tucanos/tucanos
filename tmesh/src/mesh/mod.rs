...
pub trait Mesh<const D: usize>: Send + Sync + Sized {
    ...
    fn tag_internal_faces(
        ...
        for (f, (_, ids)) in all_faces {
            if ids.len() > 1 {
                let tags = ids.iter().map(|&i| self.etag(i)).collect::<FxHashSet<_>>();
                if tags.len() > 1 && !tagged_faces.contains_key(f) {
                    n += 1;
                    let mut tmp = tags.into_iter().collect::<Vec<_>>();
                    let mut f = *f;
                    ...
                    let mut found = false;
                    for (t, tmp) in &res {
                        if tmp.iter().zip(tags.iter()).all(|(&a, &b)| a == b) {
    ...
    fn check(&self, all_faces: &FaceConnectivity<<Self::C as Simplex>::FACE>) -> Result<()> {
        ...
        for (f, (_, ids)) in all_faces {
            if ids.len() > 1 {
                ...
                if !same_tags && !tagged_faces.contains(f) {
                    let tags = ids.iter().map(|&i| self.etag(i)).collect::<Vec<_>>();
                    return Err(Error::from(&format!(
                        "Internal boundary face {f:?} not tagged ({tags:?})"
                    )));
                }
