...
pub fn ball_mesh<M: Mesh<3, C = Tetrahedron<impl Idx>>>(r: f64, n: usize) -> M {
    ...
    res.add_elems_and_tags(
        [
            (Tetrahedron::new(6, 0, 1, 2), 1),
            (Tetrahedron::new(6, 0, 2, 3), 1),
            (Tetrahedron::new(6, 0, 3, 4), 1),
            (Tetrahedron::new(6, 0, 4, 1), 1),
            (Tetrahedron::new(6, 5, 2, 1), 1),
            (Tetrahedron::new(6, 5, 3, 2), 1),
            (Tetrahedron::new(6, 5, 4, 3), 1),
            (Tetrahedron::new(6, 5, 1, 4), 1),
        ]
        .iter()
        .copied(),
    );
...
mod tests {
    use crate::{
        Vert3d, assert_delta,
        mesh::{
            BoundaryMesh3d, GSimplex, GradientMethod, Mesh, Mesh3d, ball_mesh, bandwidth, box_mesh,
