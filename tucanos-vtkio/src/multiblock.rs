...
impl<'a> MultiBlockWriter<'a> {
    ...
    pub fn new(path: &'a Path) -> Self {
        ...
        assert!(
            check_xml(&str_path),
            "Unauthorized character found in: {str_path}"
        );
    ...
    pub fn add_block<W: crate::Writer>(
        ...
        let name = name.into();
        if !check_xml(&name) || name.contains('.') || name.contains('/') || name.contains('\\') {
            return Err(io::Error::new(
                io::ErrorKind::InvalidInput,
                format!("Invalid character or path separator in block name: {name}"),
            ));
        }
...
/// Validates that a string slice does not contain characters requiring XML entity escaping.
fn check_xml(input: &str) -> bool {
    !input.contains(['&', '<', '>', '"', '\''])
}
