...
pub struct DDRemeshingInfo {
    info: RemeshingInfo,
    partition_time: f64,
    partition_quality: f64,
    partitions: Vec<RemeshingInfo>,
    interface: Option<Box<DDRemeshingInfo>>,
}

impl DDRemeshingInfo {
...
impl<const D: usize, E: Elem> DomainDecomposition<D, E> {
    ...
    pub fn remesh<M: Metric<D>, G: Geometry<D>>(
        ...
        geom: &G,
        params: RemesherParams,
        dd_params: DDRemeshingParams,
    ) -> Result<(SimplexMesh<D, E>, DDRemeshingInfo)> {
        ...
        let mut ifc = if let Some(dd_params) = dd_params.next(ifc.n_verts()) {
            let mesh = ifc;
            ...
            info.interface = Some(Box::new(DDRemeshingInfo {
                info: RemeshingInfo {
                    n_verts_init,
...
mod tests {
    ...
    fn test_domain_decomposition_2d(debug: bool, ptype: PartitionType) -> Result<()> {
        ...
        let dd_params = DDRemeshingParams::new(2, 1, 0);
    ...
    fn test_domain_decomposition_3d(debug: bool, ptype: PartitionType) -> Result<()> {
        ...
        let dd_params = DDRemeshingParams::new(2, 2, 0);
        let (mut mesh, _) = dd.remesh(&m, &NoGeometry(), RemesherParams::default(), dd_params)?;
