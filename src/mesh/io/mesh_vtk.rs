use crate::{
    mesh::{Elem, QuadraticElem, QuadraticMesh, SimplexMesh},
    Result,
};
use log::debug;
...
impl<QE: QuadraticElem> QuadraticMesh<QE> {
    pub fn write_vtk(
        ...
        debug!("Write {file_name}");
        let connectivity = self.tris().flatten().map(u64::from).collect();
        let offsets = (0..self.n_tris())
            .map(|i| u64::from(QE::N_VERTS * (i + 1)))
            .collect();
        let cell_type = match QE::N_VERTS {
            6 => CellType::HigherOrderTriangle,
            3 => CellType::HigherOrderEdge,
            _ => unreachable!(),
        };
        let mut point_data = Vec::new();
        if let Some(vertex_data) = vertex_data {
        ...
        let tag_data = IOBuffer::I32(self.etags().collect());
        #[cfg(not(any(feature = "32bit-tags", feature = "64bit-tags")))]
        let tag_data = IOBuffer::I16(self.tritags().collect());
        cell_data.push(Attribute::DataArray(DataArrayBase {
            name: String::from("tag"),
            elem: ElementType::Scalars {
                num_comp: 1,
        ...
        if let Some(elem_data) = elem_data {
            for (name, arr) in &elem_data {
                debug!("Write element data {name}");
                let num_comp = arr.len() / self.n_tris() as usize;
                cell_data.push(Attribute::DataArray(DataArrayBase {
                    name: name.to_string(),
                    elem: ElementType::Scalars {
                        num_comp: num_comp as u32,
        ...
        let vtk = Vtk {
            ...
            data: DataSet::inline(UnstructuredGridPiece {
                ...
                cells: Cells {
                    ...
                    types: vec![cell_type; self.n_tris() as usize],
                },
                data: Attributes {
                    point: point_data,
                    cell: cell_data,
