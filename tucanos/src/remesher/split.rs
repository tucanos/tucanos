...
impl<const D: usize, C: Simplex, M: Metric<D>> Remesher<D, C, M> {
    /// Attempts to split a single edge, handling standard splits, extensions, and sliver removal.
    /// Return true
    fn attempt_edge_split<G: Geometry<D>>(
        &mut self,
        dbg: bool,
        ...
        loop {
            ...
            for i_edge in indices {
                ...
                if length > params.l {
                    ...
                    self.attempt_edge_split(dbg, edg, params, geom, &mut cavity, &mut num_ops)?;
                } else {
