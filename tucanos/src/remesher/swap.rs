...
impl<T: Idx, const D: usize, C: Simplex<T>, M: Metric<D>> Remesher<T, D, C, M> {
    ...
    fn try_swap<G: Geometry<D>>(
        ...
        let local_i0 = local_edg[0].try_into().unwrap();
        let local_i1 = local_edg[1].try_into().unwrap();
        ...
        for n in 0..cavity.n_verts().try_into().unwrap() {
            ...
            if !cavity.tagged_faces.is_empty() {
                ...
                if !cavity
                    .tagged_faces()
                    .any(|(f, _)| f.contains(n.try_into().unwrap()))
                {
                    continue;
                }
            }

            let ftype = FilledCavityType::ExistingVertex(n.try_into().unwrap());
        ...
        if succeed {
            trace!("Swap from {vx}");
            let ftype = FilledCavityType::ExistingVertex(vx.try_into().unwrap());
            let filled_cavity = FilledCavity::new(cavity, ftype);
    ...
    pub fn swap<G: Geometry<D>>(
        ...
        loop {
            ...
            let mut n_swaps = T::ZERO;
            let mut n_fails = T::ZERO;
            let mut n_ok = T::ZERO;
            for edg in edges {
                let res = self.try_swap(edg, params, params.max_angle, &mut cavity, geom)?;
                match res {
                    TrySwapResult::CouldNotSwap => n_fails += T::ONE,
                    TrySwapResult::CouldSwap => n_swaps += T::ONE,
                    _ => n_ok += T::ONE,
                }
            }
            ...
            if n_swaps == T::ZERO || n_iter == params.max_iter {
                if debug {
                    self.check().unwrap();
                }
                return Ok(n_iter);
