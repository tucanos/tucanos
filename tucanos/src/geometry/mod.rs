...
use crate::{
    ...
    mesh::{SimplexMesh, SubSimplexMesh, Topology},
};
use log::{debug, warn};
...
pub trait Geometry<const D: usize>: Send + Sync {
    ...
    fn project_vertices<T: Idx, C: Simplex<T>>(&self, mesh: &mut SimplexMesh<T, D, C>) -> f64 {
        let vtags = mesh.get_vertex_tags().unwrap().to_vec();

        let mut d_max = 0.0;
        for (p, tag) in mesh.verts_mut().zip(vtags.iter()) {
    ...
    /// Compute the max distance between the face centers and the geometry normals
    fn max_distance<T: Idx, C2: Simplex<T>>(&self, mesh: &SimplexMesh<T, D, C2>) -> f64 {
        let mut d_max = 0.0;
        for (gf, tag) in mesh.gfaces().zip(mesh.ftags()) {
            let mut c = gf.center();
            let d = self.project(&mut c, &(C2::FACE::DIM as Dim, tag));
            d_max = f64::max(d_max, d);
        }
        d_max
    }
    ...
    fn max_normal_angle<T: Idx, C2: Simplex<T>>(&self, mesh: &SimplexMesh<T, D, C2>) -> f64 {
        let mut a_max = 0.0;
        for (gf, tag) in mesh.gfaces().zip(mesh.ftags()) {
            if tag > 0 {
                let c = gf.center();
                let n = gf.normal();
                let a = self.angle(&c, &n, &(C2::FACE::DIM as Dim, tag));
...
impl<const D: usize> LinearPatchGeometry<D> {
    ...
    pub fn new<T: Idx, C: Simplex<T>>(mesh: &SimplexMesh<T, D, C>) -> Self {
...
struct LinearPatchGeometryWithCurvature<T: Idx, const D: usize, C: Simplex<T>> {
    /// The face mesh
...
impl<T: Idx, const D: usize, C: Simplex<T>> LinearPatchGeometryWithCurvature<T, D, C>
where
    SimplexMesh<T, D, C>: HasCurvature<D>,
{
    /// Create a `LinearPatchGeometry` from a `SimplexMesh`
    pub fn new(mut mesh: SimplexMesh<T, D, C>) -> Self {
        let tree = ObjectIndex::new(&mesh);
        mesh.fix();
    ...
    pub fn write_curvature(&self, fname: &str) -> Result<()> {
        let mut writer = VTUFile::from_mesh(&self.mesh, VTUEncoding::Binary);
...
/// doc TODO
pub struct LinearGeometry<T: Idx, const D: usize, C: Simplex<T>>
where
    SimplexMesh<T, D, C>: HasCurvature<D>,
{
    /// The surface patches
    patches: FxHashMap<Tag, LinearPatchGeometryWithCurvature<T, D, C>>,
    /// The edges
    edges: FxHashMap<Tag, LinearPatchGeometry<D>>,
}

impl<T: Idx, const D: usize, C: Simplex<T>> LinearGeometry<T, D, C>
where
    SimplexMesh<T, D, C>: HasCurvature<D>,
{
    /// Create a `LinearGeometry` for the boundary of `mesh` (with positive tags) from a
    /// `SimplexMesh` representation of the boundary
    pub fn new<C2: Simplex<T>>(
        mesh: &SimplexMesh<T, D, C2>,
        mut bdy: SimplexMesh<T, D, C>,
    ) -> Result<Self> {
        assert!(C2::DIM >= C::DIM);
        ...
        for tag in face_tags.iter().copied() {
            ...
            let submesh = SubSimplexMesh::new(&bdy, |t| t == tag).mesh;
            assert_ne!(
                submesh.n_verts(),
                T::ZERO,
                "Geometry mesh empty for tag {tag}"
            );
        ...
        if C::DIM == 2 {
            ...
            let (bdy_edges, _) = bdy.boundary::<SimplexMesh<T, D, C::FACE>>();
            ...
            for tag in edge_tags {
                ...
                if let Some(mesh_topo_node) = mesh_topo_node {
                    let submesh = SubSimplexMesh::new(&bdy_edges, |t| t == tag).mesh;
...
impl<T: Idx, const D: usize, C: Simplex<T>> Geometry<D> for LinearGeometry<T, D, C>
where
    SimplexMesh<T, D, C>: HasCurvature<D>,
{
    fn check(&self, _topo: &Topology) -> Result<()> {
    ...
    fn angle(&self, pt: &Vertex<D>, n: &Vertex<D>, tag: &TopoTag) -> f64 {
        ...
        let n_ref = patch
            .mesh
            .gelem(&patch.mesh.elem(idx.try_into().unwrap()))
            .normal();
...
mod tests {
    ...
    fn test_stl() -> Result<()> {
        ...
        let geom: SimplexMesh<u32, 3, Triangle<u32>> = read_stl("cube2.stl")?;
        ...
        let mut p = Vert3d::new(2., 0.5, 0.5);
        let d = geom.project(&mut p, &(2, 1));
        assert!(f64::abs(d - 1.) < 1e-12);
        assert!((p - Vert3d::new(1., 0.5, 0.5)).norm() < 1e-12);

        let mut p = Vert3d::new(0.5, 0.75, 0.5);
        let d = geom.project(&mut p, &(2, 1));
        assert!(f64::abs(d - 0.25) < 1e-12);
        assert!((p - Vert3d::new(0.5, 1., 0.5)).norm() < 1e-12);
    ...
    fn test_linear_geometry_2d() -> Result<()> {
        ...
        let mut pt = Vert2d::new(0.75, 0.5);
    ...
    fn test_linear_geometry_3d() -> Result<()> {
        ...
        let mut pt = Vert3d::new(0.75, 0.5, 0.25);
