...
use tucanos::{
    ...
    remesher::{Remesher, RemesherParams, RemeshingStep, Stats},
};
...
fn main() -> Result<()> {
    ...
    let label = "old_4";
    let mut params = RemesherParams::new(25.0, 4);
    ...
    let fname = format!("cybe-cylinder_{label}_0.vtu");
    ...
    for i_step in 0..n_steps {
        ...
        let now = Instant::now();
        let topo = MeshTopology::new(&mesh);
        let fname = format!("cybe-cylinder_{label}_{i_step}.meshb");
        println!("  write {fname}");
        mesh.write_meshb(&fname)?;
        let fname = format!("cybe-cylinder_{label}_{i_step}.solb");
        println!("  write {fname}");
        let m = metric.iter().copied().flatten().collect::<Vec<_>>();
        mesh.write_solb(&m, &fname)?;
        ...
        let fname = format!("cybe-cylinder_{label}_{}.vtu", i_step + 1);
        println!("  write {fname}");
        mesh.write_vtk(fname)?;
        ...
        println!("  l: {stats_l}");
        if i_step == n_steps - 1 {
            let fname = format!("cybe-cylinder_{label}_q.dat");
            println!("  write {fname}");
            write_dat_file(&remesher.qualities(), &fname)?;
            let fname = format!("cybe-cylinder_{label}_l.dat");
            println!("  write {fname}");
