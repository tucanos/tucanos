...
macro_rules! create_dual_mesh {
    ($pyname: ident, $name: ident, $mesh: ident, $dim: expr, $face_dim: expr) => {
        #[doc = concat!("Python binding for ", stringify!($name))]
        #[pyclass]
        pub struct $pyname(pub(crate) $name<Idx>);
        ...
        impl $pyname {
            ...
            pub fn get_elems<'py>(
                ...
                for e in self.0.elems() {
                    for (f, o) in e {
                        e2f.push(f.try_into().unwrap());
                        e2f_orient.push(o);
                    }
                    ptr.push(e2f.len().try_into().unwrap());
                }
            ...
            pub fn get_faces<'py>(&self, py: Python<'py>) -> PyResult<Bound<'py, PyArray2<Idx>>> {
                PyArray::from_vec(
                    py,
                    self.0
                        .faces()
            ...
            pub fn elem_n_verts<'py>(&self, py: Python<'py>) -> Bound<'py, PyArray1<Idx>> {
                let res = (0..self.0.n_elems())
                    .map(|i| self.0.elem_n_verts(i).try_into().unwrap())
                    .collect::<Vec<_>>();
            ...
            pub fn elem<'py>(
                ...
                py: Python<'py>,
                i: usize,
            ) -> (Bound<'py, PyArray1<Idx>>, Bound<'py, PyArray1<bool>>) {
                let e = self.0.elem(i);
                let faces = e
                    .clone()
                    .into_iter()
                    ...
                    .collect::<Vec<_>>();
                let orient = e.into_iter().map(|(_, o)| o).collect::<Vec<_>>();
                (PyArray::from_vec(py, faces), PyArray::from_vec(py, orient))
            }

            /// Get the `i`th face
            pub fn face<'py>(&self, py: Python<'py>, i: usize) -> Bound<'py, PyArray1<Idx>> {
                let f = self.0.face(i);
                PyArray::from_vec(py, f.map(|x| x.try_into().unwrap()).collect())
            }
        }
    };
}

create_dual_mesh!(PyDualMesh2d, DualMesh2d, PyMesh2d, 2, 2);
create_dual_mesh!(PyDualMesh3d, DualMesh3d, PyMesh3d, 3, 3);
